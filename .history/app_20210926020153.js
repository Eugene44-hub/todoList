...
class Task {
    ...
    static displayfromLs() {
        ...
        const hrsLs = JSON.parse(localStorage.getItem('tasks'));
        const minLs = JSON.parse(localStorage.getItem('tasks'));
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `<div class=timeTo>
            <p>Time To</p>
            <p><span id="hrs">${hrsLs[i]}Hours</span>:<span id="mins">${callback.minutes}Minutes</span></p>
        </div>
