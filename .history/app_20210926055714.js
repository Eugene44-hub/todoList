...
const field = documentquerySelector('fieldset')
class Task {
    constructor(input) {
    ...
    displayTask(callback) {
        ...
        list.innerHTML += `<fieldset><legend><p>On <span>${date.value}</span> <span id="hrs">By ${callback.hours}</span>:<span id="mins">${callback.minutes}</span></p></legend>
        <li>${this.input}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i>
        </li>
        </fieldset>
        `;
    ...
    static displayfromLs() {
        ...
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `
            <fieldset><legend><p>On <span>${datesLS[i]}</span> <span id="hrs">By ${hrsLs[i]}</span>:<span id="mins">${minLs[i]}</span></p></legend><li>${fromLs[i]}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i></li>
            </fieldset>
            `
        }
