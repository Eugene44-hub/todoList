...
class Task {
    ...
    addTaskToLS(hour, minute) {
        ...
        localStorage.setItem('minutes', JSON.stringify(dates))
    ...
    static displayfromLs() {
        ...
        const datesLS = JSON.parse(localStorage.getItem('minutes'));
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `
            <li><fieldset> <legend><p>On <span>${date.value}</span> <span id="hrs">By ${hrsLs[i]}</span>:<span id="mins">${minLs[i]}</span></p>
            </legend>
