...
class Task {
    ...
    displayTask(callback) {
        ...
        list.innerHTML += `
        <li><fieldset> <legend><p><span id="hrs">By ${callback.hours}</span>:<span id="mins">${callback.minutes()}</span></p>
    ...
    static displayfromLs() {
        ...
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `
            <li><fieldset> <legend><p><span id="hrs">By ${hrsLs[i]}</span>:<span id="mins">${minLs[i]}</span></p>
            </legend>
    ...
    setTimer() {
        ...
        const timeTo = new Date(`${date.value}T${time.value}:00Z`).toString()
        ...
        return {
            ...
            minutes: function() {
                if (timeTo.getMinutes < 10) {

                    return '0' + timeTo.getMinutes().toString()
                }
                return timeTo.getMinutes()
            }
        }
