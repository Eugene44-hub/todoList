...
class Task {
    ...
    static displayfromLs() {
        ...
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `
            <li><fieldset><legend><p>On <span>${datesLS[i]}</span> <span id="hrs">By ${hrsLs[i]}</span>:<span id="mins">${minLs[i]}</span></p></legend>${fromLs[i]}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i></fieldset></li>`
        }
