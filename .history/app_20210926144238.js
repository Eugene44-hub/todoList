...
class Task {
    ...
    displayTask(callback) {
        ...
        list.innerHTML += `<li>${this.input}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i><label for=date>On ${date.value} By ${callback.hours}${callback.minutes}</label></li><label for=date>On ${date.value} By ${callback.hours}${callback.minutes}</label>`;
    ...
    static displayfromLs() {
        ...
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `<li>${fromLs[i]}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i><label for=date>On ${datesLS[i]} By ${hrsLs}:${minLs}</label></li>`
        }
    ...
    static TimeUp() {
        ...
        if (timeTo.getTime() === currentDate.getTime()) {
            Swal({
                icon: 'notification',
                title: 'its time for',
                text: 'a current Task'
