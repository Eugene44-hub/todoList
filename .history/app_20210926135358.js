...
class Task {
    ...
    displayTask(callback) {
        ...
        list.innerHTML += `<li><fieldset>
        <legend>
        <p>On <span>${date.value}</span> <span id="hrs">By ${callback.hours}</span>:<span id="mins">${callback.minutes}</span></p>
        </legend>
        <span id="task">${this.input}</span>
    ...
    taskComplete(task) {
        ...
        console.log(task.childNodes[0].childNodes[3].textContent)
    }
    static displayfromLs() {
        ...
        for (let i = 0; i < fromLs.length; i++) {
            list.innerHTML += `<li><fieldset>
            <legend>
            <p>On <span>${datesLS[i]}</span> <span id="hrs">By ${hrsLs[i]}</span>:<span id="mins">${minLs[i]}</span></p>
            </legend>
            <span id="task">${fromLs[i]}</span>
            <i class="fas fa-trash-alt" id="delete"></i>
            <i class="fas fa-check-square" id="checked"></i>
        </fieldset>
        </li>
    ...
    deleteFromLs(task) {
        ...
        for (let i = 0; i < tasks.length; i++) {
            if (tasks[i] === task.childNodes[3].textContent) {
                tasks.splice([i], 1)
                hours.splice([i], 1)
                minutes.splice([i], 1)
                dates.splice([i], 1)
        ...
        console.log(task.childNodes[3].textContent)
    }
    setTimer() {
    ...
    document.body.addEventListener('click', e => {
        ...
        if (e.target.id === 'delete') {
            if (confirm('are you sure you want to delete'))
                task.deleteTask(e.target.parentElement, e.target.parentElement.parentElement);

        }
        if (e.target.id == 'checked') {
            task.taskComplete(e.target.parentElement.parentElement);
...
const liTask = document.querySelector('#checked');
console.log(liTask.parentElement.parentElement)
...
// <li>${fromLs[i]}<i class="fas fa-trash-alt" id="delete"></i><i class="fas fa-check-square" id="checked"></i></li>